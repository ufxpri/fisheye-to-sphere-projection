...
from math import sin, cos, acos, tan, atan2, pi, sqrt

import cv2
import numpy as np

image = cv2.imread('image.jpg')
# image = cv2.imread('cube_orig.jpg')
# image = cv2.imread('split.png.jpg')
# image = cv2.resize(image, None, fx=0.2, fy=0.2, interpolation=cv2.INTER_LINEAR)
image_width = len(image[0])
image_height = len(image)
remap_width = 200
remap_height = 200
circle_center = (int(image_width/2), int(image_height/2))
circle_radius = int(image_height/2)
...
print('input image size: ', len(image), len(image[0]))
...
if __name__ == "__main__":
    ...
    cv2.createTrackbar('angle','remap',0,100,make_map)

    i = 0
    while True:
        remap = np.zeros((remap_height,remap_width,3), np.uint8)

        remap = cv2.remap(image, map_x, map_y, cv2.INTER_LINEAR)
